...
> 每个核心模块生成一份核心日志文档，保存到 `docs/modules/{模块名}/` 目录下，文件名为 `{模块名}_核心日志.md`。
> ⚠️ **仅核心模块需要生成**此文档。核心模块 = 用户可达界面所在模块 + 后台运行能力所在模块。非核心模块（构建脚本、纯工具库、测试模块等）不需要日志文档。
...
| 非核心（排除） | 构建脚本、Gradle 插件、纯工具库（无UI无后台）、测试模块 | — |

> **本模块判定结果**：{核心模块 / 非核心模块}，判定依据：{简述满足的条件}
