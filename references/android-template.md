...
| -------------- | --------- | ---- | ----------- | --------- | -- | ------ |
| news\_resource | id        | LONG | PRIMARY KEY | -         | ✅  | 主键     |
| news\_resource | title     | TEXT | NOT NULL    | -         | -  | 标题     |
| news\_resource | topic\_id | LONG | NOT NULL    | topic(id) | ✅  | 关联话题ID |
...
| ----- | --- | -------------- | --------- | ---- | -------------- |
| topic | id  | news\_resource | topic\_id | 一对多  | CASCADE DELETE |

#### 索引设计

| 表名             | 索引名            | 字段        | 类型   | 说明     |
| -------------- | -------------- | --------- | ---- | ------ |
| news\_resource | idx\_topic\_id | topic\_id | 普通索引 | 加速话题查询 |
| user           | uk\_email      | email     | 唯一索引 | 邮箱唯一约束 |
...
flowchart TB
    subgraph App[应用启动]
        LAUNCH["启动App<br>Application.onCreate()<br>NowInAndroidApp.kt:45"]
        INIT["初始化模块<br>HiltApplication.triggerInit()<br>core/HiltHelper.kt:23"]
        LOAD["加载用户数据<br>UserRepo.getCurrentUser()<br>data/UserRepo.kt:89"]
        LAUNCH --> INIT
        INIT --> LOAD
    end
    subgraph HomeFlow[首页流程]
        HOME["展示首页<br>MainActivity.onCreate()<br>ui/MainActivity.kt:67"]
        FORYOU["为你推荐<br>ForYouViewModel.loadFeeds()<br>feature/foryou/ForYouVM.kt:56"]
        TOPIC["话题列表<br>TopicViewModel.loadTopics()<br>feature/topic/TopicVM.kt:45"]
        INTEREST["关注内容<br>InterestVM.loadFollowed()<br>feature/interest/InterestVM.kt:34"]
        HOME --> FORYOU
        HOME --> TOPIC
        HOME --> INTEREST
    end
    subgraph FeatureFlow[功能流程]
        DETAIL["内容详情<br>NewsDetailScreen.render()<br>ui/detail/NewsDetailScreen.kt:78"]
        TOPIC_DETAIL["话题详情<br>TopicDetailScreen.render()<br>ui/topic/TopicDetailScreen.kt:56"]
        BOOKMARK["收藏<br>BookmarkVM.toggleBookmark()<br>feature/bookmark/BookmarkVM.kt:89"]
        SHARE["分享<br>ShareUtil.shareNews()<br>util/ShareUtil.kt:34"]
        DETAIL --> BOOKMARK
        DETAIL --> SHARE
    end
    ...
    subgraph Background[后台流程]
        SYNC["数据同步<br>SyncManager.startSync()<br>core/SyncManager.kt:56"]
        NOTIFY["推送通知<br>FcmService.onMessageReceived()<br>service/FcmService.kt:34"]
        SYNC --> NOTIFY
    end
...
flowchart TB
    subgraph UI[界面层]
        FORYOU_SCREEN["ForYouScreen<br>ForYouScreen() Composable<br>feature/foryou/ForYouScreen.kt:34"]
    end
    subgraph VM[ViewModel层]
        FORYOU_VM["ForYouViewModel<br>loadFeeds()/refresh()<br>feature/foryou/ForYouVM.kt:56"]
    end
    subgraph UseCase[业务逻辑层]
        GET_RECOMMEND["GetRecommendUseCase<br>execute(userId)<br>domain/GetRecommendUseCase.kt:23"]
        FILTER_CONTENT["FilterContentUseCase<br>applyFilters()/deduplicate()<br>domain/FilterContentUseCase.kt:45"]
        SORT_CONTENT["SortContentUseCase<br>sortByScore()/rank()<br>domain/SortContentUseCase.kt:67"]
    end
    subgraph Data[数据层]
        REPO["ContentRepository<br>getFeeds()/saveFeeds()<br>data/ContentRepo.kt:78"]
        LOCAL_DS["LocalDS (Room)<br>FeedDao.query()/insert()<br>data/local/FeedDao.kt:34"]
        REMOTE_DS["RemoteDS (Retrofit)<br>NewsApi.getFeeds()<br>data/remote/NewsApi.kt:90"]
    end
