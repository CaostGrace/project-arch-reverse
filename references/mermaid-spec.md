...
> 生成时需要逐块扫描所有 Mermaid 图表，自动检测并修复以下 10 种语法错误模式。双门禁机制：门禁1（CP4生成时）+ 门禁2（CP6自检时）。

### 检测规则总览

| # | 错误模式 | 影响图表 | 检测正则 | 修复方法 |
|---|---------|---------|---------|---------|
| 1 | 消息含 `<>` 尖括号 | 时序图 | `->+.*<(?!br>).*>` | 双引号包裹消息 |
| 2 | 消息含 `#` 井号 | 时序图 | `->+.*#` | 双引号包裹消息 |
| 3 | loop/alt 块内空行 | 时序图 | `loop.*\n\s*\n(?!\s*end)` | 删除空行 |
| 4 | 消息双引号冲突 | 时序图 | `".*".*"` | 内部改单引号 |
| 5 | 节点文本裸括号 `()` | 流程图 | `\[(?!").*\(.*\).*\]` | 加双引号 |
| 6 | 圆柱体 `[( )]` 冲突 | 流程图 | `\[\(.*\).*\)\]` | 改为矩形 `[""]` |
| 7 | 未定义节点引用 | 流程图 | 箭头目标 ID 未声明 | 补充节点定义 |
| 8 | subgraph 含特殊字符 | 流程图 | `subgraph.*[^a-zA-Z0-9_]` | 替换为下划线 |
| 9 | ER 实体名含连字符 | ER图 | `\w-\w` | 替换为下划线 |
| 10 | participant 含 `::` | 时序图 | `participant.*::.*(?!"$)` | 双引号包裹别名 |

### 详细规则

#### 规则1: 消息含尖括号 `<>`
```
问题: A->>B: List<Item>          → 尖括号被当作 Mermaid 语法
检测: 消息文本含 < 或 > 且未用双引号包裹
修复: A->>B: "List<Item>"       (或用 &lt; &gt; 转义)
```

#### 规则2: 消息含井号 `#`
```
问题: A->>B: get #1 user        → # 被当作注释或特殊符号
检测: 消息文本含 # 且未用双引号包裹
修复: A->>B: "get #1 user"
```

#### 规则3: loop/alt/opt 块内空行
```
问题:
  loop 每个版本
      A->>B: step1
...
修复: 删除 loop/alt/opt 块内的孤立空行
```

#### 规则4: 消息内双引号冲突
```
问题: A->>B: "he said "hello""  → 内部双引号结束包裹
修复: A->>B: "he said 'hello'"  (内部改单引号)
```

#### 规则5: 流程图节点文本裸括号
```
问题: A[LoginActivity.login()]  → () 被当作圆角矩形语法
修复: A["LoginActivity.login()"] (添加双引号)
```

#### 规则6: 圆柱体形状冲突
```
问题: DB[(User(id, name))]      → 内括号与圆柱体语法冲突
修复: DB["User(id, name)"]       (改为矩形)
```

#### 规则7: 未定义节点引用
```
问题: A --> B (但 B 未用 B[text] 定义)
检测: 提取所有箭头终点 ID，对照已声明的节点 ID
修复: 补充 B["占位"] 定义在流程图顶部
```

#### 规则8: subgraph 标识符特殊字符
```
问题: subgraph "中文模块"        → 部分 Mermaid 版本不支持
修复: subgraph Module1           (中文改为注释 #34;中文模块)
```

#### 规则9: ER 图实体名含连字符
```
问题: news-resource { int id }   → - 与关系语法 -- 冲突
修复: NEWS_RESOURCE { int id }   (替换为下划线)
```

#### 规则10: participant 别名含 `::`
```
问题: participant A as org::Module::Class  → :: 未引号包裹时解析异常
修复: participant A as "org::Module::Class" (双引号包裹)
```
...
```
对每个 mermaid 代码块:
  1. 判断图表类型 (sequenceDiagram / flowchart / erDiagram / classDiagram)
  2. 应用该类型的所有规则（正则扫描）
  3. 发现错误 → 执行修复 → 重新扫描确认通过
  4. 无法修复 → 标记跳过 → 记录到质检报告
  5. 输出: "✅ Mermaid 块 #N 通过" 或 "⚠️ Mermaid 块 #N 跳过 M 项"
```
...
```
┌─────────────────────────────────────────┐
│ Mermaid 语法质检: 共 N 块 / 通过 P 块    │
│ ✅ 自动修复: X 个问题                     │
│ ⚠️ 跳过: Y 个问题 (无法自动修复)          │
│                                         │
│ 跳过项:                                  │
│ - 文档§X 时序图: 未定义节点 B → 已补充   │
│   "占位"定义，请人工确认节点名称           │
│ - 文档§Y ER图: 实体名 "user-log" →       │
│   已改为 USER_LOG                        │
└─────────────────────────────────────────┘
```
- `ClassA --> ClassB` - 关联关系
