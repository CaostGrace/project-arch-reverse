...
### 格式规范
```
节点文本格式：[步骤名]<br/>⚙️ 方法名<br/>📁 文件路径:行号
```

```mermaid
flowchart TB
    subgraph App[表现层]
        List[列表展示<br/>⚙️ ListController.index()<br/>📁 modules/list/ListController.java:34]
        Search[搜索输入<br/>⚙️ SearchService.onQuery()<br/>📁 modules/search/SearchService.ts:56]
        Detail[详情展示<br/>⚙️ DetailHandler.render()<br/>📁 modules/detail/DetailHandler.py:89]
    end
    subgraph Feature[业务层]
        F1[核心逻辑A<br/>⚙️ ModuleAService.execute()<br/>📁 services/ModuleAService.java:112]
        F2[核心逻辑B<br/>⚙️ ModuleBService.process()<br/>📁 services/ModuleBService.ts:45]
    end
    subgraph Core[数据层]
        C1[数据访问<br/>⚙️ DataRepository.query()<br/>📁 data/DataRepository.java:78]
        C2[缓存管理<br/>⚙️ CacheService.get()<br/>📁 core/CacheService.py:34]
    end
...
- 使用 `<br/>` 在节点内换行
- 第一行：步骤名称
- 第二行：`⚙️ ClassName.methodName(params)` 核心方法
- 第三行：`📁 相对路径:行号` 代码位置
- 节点文本过长时可用简写 `A["长文本内容"]`
