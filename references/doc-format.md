...
> ⚠️ **以上自检清单为人工目检辅助**。生成时质检（门禁1）和自检时质检（门禁2）会自动执行 Mermaid 10条规则、表格列数、代码块、链接等校验。详见 `references/mermaid-spec.md` 自动检测规则。

## 质检报告格式 ★

### 生成时质检报告（门禁1，CP4→CP5间输出）

```
┌─────────────────────────────────────────────────────────┐
│  生成时质检报告                                          │
├─────────────────────────────────────────────────────────┤
│  Mermaid 语法: N 块 / 通过 P 块                          │
│    ✅ 自动修复: X 个问题 (尖括号/裸括号/连字符...)        │
│    ⚠️  跳过: Y 个问题 (无法自动修复)                     │
│                                                         │
│  表格校验: M 个表格 / 通过 N 个                           │
│    ✅ 自动修复: P 个列数不一致                            │
...
以下问题无法自动修复，建议通过 CP7 人工处理：

| # | 文档 | 位置 | 问题类型 | 跳过原因 | 建议 |
|---|------|------|---------|---------|------|
| 1 | 根文档.md | §12.2 | 表格列数 | 索引表缺少F-005所在列的数据源 | 补充对应模块的§6档案 |
| 2 | auth模块架构.md | §6 | Mermaid | 参与者数量不足(3<4) | 功能简单，确认是否可接受 |
```
...
| `[x]` | 完成 | 生成成功 |
| `[!]` | 跳过 | **仅限不可恢复错误**（源码删除/磁盘不足/编码异常）。严禁因模块过多或用户认为够了而使用 |
