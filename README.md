...
- **核心日志提取**：扫描源代码中的日志语句，为每个核心功能生成日志清单
- **强制生成所有模块文档**：不可省略任何模块
- **自检验证**：生成后自动检查文档完整性、依赖关系正确性、模块文档生成状态、核心日志文档完整性（最多3次循环）
...
```
检查点0（增量检测） → 检查点1（项目类型确认）
                          ↓
                  【阶段: 项目扫描】 → 【阶段: 文档生成】
                          ↓
检查点2（结果确认） → 检查点2.5（自检验证，必须，最多3次循环）
                          ↓
              检查点3（人工更正，可选）
```
...
> ⚠️ **重要**：所有模块的架构文档都必须生成，不可省略！核心模块的日志文档也必须生成。
...
├── README.md                   # 本文件
└── references/
    ├── INDEX.md                 # 参考文档入口索引（按用途/检查点/学习路径分类）
    ├── quick-ref.md            # 快速参考（执行预估、场景、策略、命令）
    ├── chapters-ref.md          # 章节速查表（21章+12章+5章详情）
    ...
    ├── log-extraction-guide.md  # 各语言日志提取指南
    ├── template.md              # 通用模板
...
1. **模块概述**：模块定位和职责
2. **模块 UI 组件清单**：UI框架、关键组件、UX设计要点
3. **依赖关系**：依赖的模块和库
4. **业务流程**：模块内业务流程
5. **数据流**：数据流转过程
6. **核心功能流程图**：核心功能的流程和时序图
7. **数据依赖关系**：与其他模块的数据交互
8. **依赖场景说明**：依赖其他模块的场景
9. **页面与路由**：模块内页面和路由
10. **导航调用示例**：页面跳转参数和示例
...
4. **核心日志文档完整性**：检查核心模块日志文档是否生成、日志是否覆盖所有核心功能
