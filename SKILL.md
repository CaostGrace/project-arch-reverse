...
**输入**：CP1 输出 | **输出**：依赖矩阵 + Mermaid图 + 拓扑排序分组 + 模块结构 → 进入生成阶段
...
| 5 | 导航分析 | nav*.xml / router / App.tsx 等 |
| 6 | 生成图表 | Mermaid — ⚠️ 每节点标注方法名+文件:行号（见 `references/mermaid-spec.md`） |
| 7 | 加载模板 | 根据项目类型加载 `references/{类型}-template.md`（见参考文档表格） |

> ★ **拓扑排序**：扫描阶段新增步骤 4，输出按依赖深度分层的模块分组。同层模块互不依赖，可并行生成。循环依赖组（如 A⇄B）标记为虚拟层，同层处理。
