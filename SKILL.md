...
| **B** | 读现有根文档 → 识别缺失模块 → 生成缺失模块文档 → 更新根文档 → `mkdir docs/logs/`（如不存在）→ **同步补充缺失核心日志文档**（新增模块 = 新增日志文档） |
| **C** | 对比代码vs文档时间戳 → 更新变化模块 → 新模块生成文档 → `mkdir docs/logs/`（如不存在）→ **同步更新核心日志文档**（架构变更模块 = 日志同步更新）→ **确保所有核心模块日志文档存在** |
| **D** | 基于已有模块文档重建根文档 → 补充缺失模块文档 → `mkdir docs/logs/`（如不存在）→ **同步补充所有缺失核心日志文档** |
...
> 🔴 **强制要求：每个核心功能/核心能力都必须有独立的时序图**。不允许将多个核心功能合并为一个粗略时序图。时序图必须足够详细（见详细度要求），禁止生成只有3-4步的粗略时序图。
...
| 4 | 核心日志文档完整性 + 架构变更同步 | 缺失→调用log-extraction-guide提取；覆盖不足→补充扫描；架构文档有更新但日志未同步→立即同步更新日志文档 |
