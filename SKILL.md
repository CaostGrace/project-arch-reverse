...
> codegraph 可用时，扫描阶段和数据提取按以下管道执行。MCP server 优先（`codegraph mcp` 启动 33 个工具），不可用时降级 CLI。
...
4. 查询模块间调用边 → 依赖矩阵
5. 查询跨模块调用链 → 跨模块分析候选
   CLI: codegraph audit <target>  |  codegraph context <func> -T
   MCP: 使用对应 tool 结构化查询
```
...
> 标注为"不可覆盖"的章节由手动步骤完成（导航分析等），不在 codegraph 函数图范围内。
